...
\subsection{Convolutional neural network} \todo{specificare input size (quando l'avremo scelta)}
...
\todo{Aggiungere test set, quanti sample ci sono nel training set, quanti nel validation set. Aggiungere pure da qualche parte che mancano le annotazioni per 210 identità e che queste identità le abbiamo brutalmente ignorate.}
...
Ovviamente, qualora questa rete dovesse essere utilizzata per un'applicazione reale, andrebbe aggiunto un face detector a monte del sistema di age estimation. \todo{Dire come sono stati estratti questi volti o citare qualche parte nella quale ci sia scritto}
...
Dopo le operazioni di ridimensionamento, ad ogni immagine viene sottratta la media, per ogni canale, delle immagini del dataset VGGFace2\todo{Specificare anche perché la facciamo (riguarda la convergenza, sta scritto sul paper del MIVIA)}.
...
I valori scelti empiricamente fanno sì che l'effetto dell'applicazione del filtro non sia completamente distruttivo e il risultato sia ancora riconoscibile all'occhio umano rispetto all'immagine originale. \todo{aggiungere immagine flippata e immagine originale alla carrellata di effetti}
...
Come base per il nostro addestramento abbiamo realizzato una versione di MobileNetV3Large, basata sull'implementazione già disponibile in TensorFlow\footnote{https://www.tensorflow.org/api_docs/python/tf/keras/applications/MobileNetV3Large}, nella quale i livelli più bassi, che svolgono le operazioni di estrazione delle feature dall'input, sono inizializzati con pesi appresi su ImageNet\todo{linkare/citare qualcosa}, mentre negli ultimi sei livelli della rete (il \emph{top}), che producono l'output finale della rete sulla base della feature estratte ai livelli inferiori, vi è soltanto un livello che richiede dei pesi, quello convoluzionale, che abbiamo inizializzato in maniera randomica. In particolare, per questo livello abbiamo lasciato l'inizializzazione di default prevista dall'implementazione, ovvero l'inizializzazione uniforme di Glorot, sebbene di solito si preferisca l'inizializzazione uniforme di He per layer con funzione d'attivazione ReLU\todo{aggiornare quando scegliamo modello definitivo}, in quanto le due inizializzazioni differiscono soltanto per un fattore al numeratore della varianza della distribuzione utilizzata (1 in Glorot, 2 in He), ed il fatto di non modificare l'inizializzazione ci ha permesso di non dover modificare l'implementazione della rete.\todo{dire che se la rete avesse fatto schifo saremmo comunque venuti a modificare questo parametro}

Abbiamo scelto di seguire questa strada in quanto il fatto di partire da pesi appresi su di un dataset generico e molto vasto permette di velocizzare i tempi di convergenza dell'algoritmo di addestramento\cite{miviaage}. 

Ovviamente non abbiamo potuto riutilizzare i pesi per il \emph{top} della rete in quanto ImageNet è un dataset tipicamente utilizzato per la classificazione.

Si noti infine che nel model summary visualizzato dalla nostra implementazione il terzultimo livello viene denominato \emph{Logits}. Si tratta semplicemente del nome che l'implementazione sulla quale ci siamo basati dà al livello precedente a quello di output, ma non ha nulla a che vedere con la funzione matematica \emph{logit}, è solo frutto di una scelta dei nomi a nostro parere abbastanza confusionaria.


\subsubsection{Procedura di training}
\todo{Indicare probabilità di corruzioni e probabilità di corruzioni frequenti. Indicare anche che non facciamo regolarizzazione (weight decay o regolarizzazione L1 o L2) perché c’è già la Batch Normalization nel modello e i risultati sono in linea con il paper di Age Estimation del MIVIA anche senza.}
