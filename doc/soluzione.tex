...
È stato appurato quindi che era necessario ridurre il numero di campioni. Ci siamo quindi chiesti se vi fosse qualche sbilanciamento all'interno del dataset che potevamo \todo{check periodo ipotetico} mitigare durante il nostro processo di eliminazione di dati. Abbiamo pertanto svolto una prima analisi sul genere delle identità presenti.
...
Durante la seconda fase di pre-processing, che viene effettuata a valle della data augmentation, ogni immagine viene ridimensionata alla dimensione di input della rete, e viene eseguita anche una normalizzazione.\todo{non si capisce tanto che ci riferiamo alla sottrazione delle medie} 
...
PROCEDURA DI TRAINING IN GENERALE (learning rate, numero di epoche etc.)\todo{eliminare questo}
